...
def acfun_to_csv(row):
    # 将 acfun 数据写入 csv 文件
    with open('acfun_weibo_name_with_id_002.csv', 'a', newline='') as csvfile:
...
def bili_to_csv(row):
    # 将 acfun 数据写入 csv 文件
    with open('bili_weibo_name_with_id_002.csv', 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(row)

# 获取单个acfun用户的 id
def get_acfun_id(data):
    conn = pymysql.connect(host="223.3.76.172", user="root", passwd="123", charset="utf8", db='weibo002')
    table_name ="weibouser"+table_index
    containerid_sql = 'select containerid from %s '%table_name
    containerid_sql += 'where nickname =%s;'

    print("在数据库 weibo002的 weibouser%s表查找acfun第%i 个用户..." % (table_index,data[4]))
    try:
        cur = conn.cursor()
...
def get_bili_id(data):
    ...
    print("在数据库 weibo002的 weibouser%s表查找bili第%d 个用户..."% (table_index,data[4]))
    try:
        cur = conn.cursor()
        cur.execute(containerid_sql,data[2])
        record = cur.fetchone()
...
#在database002中20个表中遍历
'''
在 weibo002中查找 name 的 id
'''
#  遍历20个表名
for i in range(0,20):
    if int(i/10)==0:
